...
interface NodeService {
  ...
  node_service_description: string | null;
...
const statusFetcher = async ([url, payload]: [string, MonitoringTarget[]]) => {
...
export default function NodeDetailPage() {
  ...
  const workspaceIdInt = parseInt(params?.workspace_id as string, 10);
  const selectedNodeId = params?.node_id as string;
  ...
  const { data: nodeDetail, isLoading: isDetailLoading, mutate: mutateNode } = useSWR<NodeDetailResponse>(
    selectedNodeId ? `/api/workspaces/${workspaceIdInt}/nodes/${selectedNodeId}` : null,
    fetcher
  );
  ...
  const targetPayload = useMemo<MonitoringTarget[]>(() => {
    ...
    // Add node itself
    targets.push({
      ip: nodeDetail.node_ip_address,
      port: nodeDetail.node_port || 0
    });
    // Add services
    nodeDetail.services.forEach(svc => {
      targets.push({
        ip: svc.node_service_ip_address,
  ...
  const { data: serviceStatusData } = useSWR<StatusApiResponse, Error, [string, MonitoringTarget[]] | null>(
  ...
  const activeNodeData = useMemo(() => {
    if (!nodeDetail || !serviceStatusData) return { label: nodeDetail?.node_label, status: 'unknown', latency: '---' };
    
    const ipNode = nodeDetail.node_ip_address;
    const portNode = nodeDetail.node_port;
    const nodeTarget = portNode > 0 ? `${ipNode}:${portNode}` : ipNode;
    
    const statusData = serviceStatusData.nodes.find(n => n.target === nodeTarget);
    
    return {
      label: nodeDetail.node_label,
      status: statusData?.status || 'unknown',
      latency: statusData?.latency || '---'
    };
  }, [nodeDetail, serviceStatusData]);
  ...
  const handleDeleteNode = useCallback(async () => {
    ...
    try {
      ...
      router.push(`/workspaces/${workspaceIdInt}/dashboard`);
    } catch (err) {
      setDeleteNodeError(err instanceof Error ? err.message : 'Terjadi kesalahan');
      setIsDeletingNode(false);
      setDeleteNodeConfirm(false);
    }
  }, [workspaceIdInt, selectedNodeId, router]);

  const hasServices = nodeDetail?.services && nodeDetail.services.length > 0;

  return (
    <div className="min-h-screen z-1 bg-base-200 font-sans lg:pl-72">
      <main className="flex-1 overflow-auto bg-base-100 p-6 md:p-8 rounded-tl-3xl shadow-2xl relative">
        <div className="max-w-full mx-auto space-y-6">
          <div className="flex items-center gap-4 border-b border-base-200 pb-4">
            <button 
              onClick={() => router.push(`/workspaces/${workspaceIdInt}/dashboard`)}
              className="btn btn-ghost btn-circle btn-sm bg-base-200 hover:bg-base-300 transition-colors"
            >
              <ArrowLeft size={18} />
            </button>
          ...
          <div className="w-full bg-base-100 rounded-3xl border border-base-300 p-0 overflow-hidden mt-0 shadow-lg">
            {!isEditingNode && (
              <div className={`p-6 ${nodeDetail ? 'bg-base-200/50' : 'animate-pulse bg-base-300'}`}>
            ...
            <div className="p-6 space-y-6 bg-base-100/50">
              ...
              <div>
                ...
                {isAddingService && selectedNodeId && nodeDetail && (
                  <div className="mb-4">
                    <AddServiceForm workspaceId={workspaceIdInt} nodeId={selectedNodeId as string} nodeIp={nodeDetail.node_ip_address} onSuccess={handleServiceAdded} onCancel={() => setIsAddingService(false)} />
                  </div>
                )}
                  ...
                  {isDetailLoading ? (
                    ...
                    nodeDetail!.services.map((svc: NodeService) => (
                      <ServiceRow key={svc.node_service_id} svc={svc}
                        workspaceId={workspaceIdInt} nodeId={selectedNodeId as string} serviceStatusData={serviceStatusData} onMutated={handleServiceAdded} />
                    ))
                  ) : (
