...
import {
  NodeEditForm, ServiceRow, AddServiceForm, MiniLatencyChart,
} from '@/src/components/shared/NodeDetail';
...
export default function NodeDetailPage() {
  ...
  const targetPayload = useMemo<MonitoringTarget[]>(() => {
    ...
    targets.push({ ip: nodeDetail.node_ip_address, port: nodeDetail.node_port || 0 });
    nodeDetail.services.forEach(svc => {
      targets.push({ ip: svc.node_service_ip_address, port: svc.node_service_port || 0 });
    });
    return targets;
  }, [nodeDetail]);
                  ...
                  {isDetailLoading ? (
                    ...
                    nodeDetail!.services.map((svc: NodeService) => (
                      <ServiceRow key={svc.node_service_id} svc={svc}
                        workspaceId={workspaceIdInt} nodeId={selectedNodeId} serviceStatusData={serviceStatusData} onMutated={handleServiceAdded} />
                    ))
                  ) : (
