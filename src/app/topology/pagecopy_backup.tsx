...
function TopologyEditor() {
  ...
  const servicePayload = useMemo<MonitoringTarget[]>(() => {
    ...
    return nodeDetail.services.map(svc => ({
      ...
      port: svc.node_service_port || 0 // Jika port tidak ada, default ke 0 (anggap ICMP)
    }));
  }, [nodeDetail]);
                  ...
                  <div className="grid grid-cols-1 gap-3 max-h-64 overflow-y-auto pr-2 custom-scrollbar">
                    {isDetailLoading || (isDetailOpen && servicePayload.length > 0 && !serviceStatusData) ? (
                      ...
                      nodeDetail.services.map((svc) => {
                        // 1. Ambil data dasar & pastikan tidak ada spasi (trim)
                        const ip = svc.node_service_ip_address?.trim();
                        const port = svc.node_service_port;

                        // 2. Logic Target Key: Harus sama dengan format API (Hanya IP jika ICMP)
                        const hasPort = port && port > 0;
                        const targetKey = hasPort ? `${ip}:${port}` : ip;

                        // 3. Cari di data monitoring real-time
                        ...
                        return (
                          <div key={svc.node_service_id} className={`flex items-center justify-between p-4 rounded-2xl border transition-all duration-500 ${isSvcOnline ? 'bg-success/5 border-success/20' : 'bg-error/5 border-error/20'
                            ...
                            <div className="flex items-center gap-4">
                              {/* Visual Port / PING */}
                              <div className={`h-10 w-10 rounded-xl flex items-center justify-center font-black text-[10px] border transition-colors ${isSvcOnline ? 'bg-success/10 text-success border-success/20' : 'bg-error/10 text-error border-error/20'
                                }`}>
                                {hasPort ? port : 'PING'}
                              </div>
                              ...
                              <div className="flex flex-col">
                                ...
                                {/* MENAMPILKAN IP ADDRESS SERVICE */}
